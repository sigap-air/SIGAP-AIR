<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-[#022448] to-[#0A3D73] px-6 py-5 flex items-center gap-4">
                @if($user->foto_profil)
                    <img src="{{ asset('storage/' . $user->foto_profil) }}"
                         alt="{{ $user->name }}" class="w-12 h-12 rounded-full ring-2 ring-white/20 object-cover bg-white">
                @else
                    <div class="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center ring-2 ring-white/20">
                        <span class="material-symbols-outlined text-white text-2xl">person</span>
                    </div>
                @endif
                <div>
                    <h1 class="text-lg font-bold text-white tracking-wide">Edit User: {{ $user->name }}</h1>
            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}"
                ...
                <div class="rounded-xl border border-dashed border-gray-300 p-4 bg-gray-50/50">
                    <p class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Ubah Password (opsional — kosongkan jika tidak ingin diubah)</p>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                ...
                {{-- Info Meta --}}
                <div class="rounded-xl bg-blue-50/50 border border-blue-100 p-4 text-xs text-gray-500 flex flex-wrap gap-x-6 gap-y-2 items-center">
                    <span class="flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">tag</span> <span class="font-semibold text-gray-700">ID:</span> #{{ $user->id }}</span>
                    <span class="flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">badge</span> <span class="font-semibold text-gray-700">NIP:</span> {{ $user->petugas?->nip ?? '—' }}</span>
                    <span class="flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">calendar_today</span> <span class="font-semibold text-gray-700">Terdaftar:</span> {{ $user->created_at?->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</span>
                </div>

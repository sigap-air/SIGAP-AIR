...
<head>
    ...
    <title>Lupa Password - SIGAP-AIR</title>
...
<section class="w-full lg:w-1/2 flex flex-col bg-surface-container-lowest justify-center px-6 md:px-20 lg:px-32 relative">
    <div class="max-w-md w-full mx-auto">
        ...
        <div class="mb-10 text-left">
            <h2 class="font-headline font-semibold text-on-surface text-3xl mb-2">Lupa Password?</h2>
            <p class="text-outline text-body-md font-medium leading-relaxed">
                Kami akan mengirimkan link untuk mengatur ulang password Anda. Silakan masukkan alamat email yang terdaftar.
            </p>
        </div>

        <!-- FLASH MESSAGE -->
        @if(session('status'))
            <div class="mb-8 text-sm font-medium text-green-700 bg-green-50 p-4 rounded-xl border border-green-200">
                {{ session('status') }}
            </div>
        @endif
        ...
        <!-- Divider & Link Kembali -->
        <div class="mt-8 flex justify-center">
            <a href="{{ route('login') }}" class="inline-flex items-center gap-2 text-secondary font-semibold text-sm hover:underline hover:text-on-secondary-fixed-variant transition-colors">
                <span class="material-symbols-outlined" style="font-size: 1.25rem; font-variation-settings: 'wght' 600;">arrow_back</span>
                Kembali ke Login
            </a>
        </div>
    </div>
